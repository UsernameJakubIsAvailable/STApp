...
function OrnamentLetter(props) {
    ...
    const changeFirstLetterToOrnamentLetterImg = () => { 
      ...
      return letterObject[firstLetter] ? <img className='ornamentLetter' src={letterObject[firstLetter]} alt='ornamentLetter' /> : <span className='firstLetter'>{firstLetter}</span>;
    }

    return(
        <>
        {changeFirstLetterToOrnamentLetterImg()}
        </>
       )
